...
---
### 🤔 REMIND
- `int(str1 in str2)` : 불리언 연산자로 나온 결과를 정수로 바꿔서 반환한다. True -> 1, False -> 0

### 💻 접근법
인사이트 : 불리언 연산자를 사용하여 문자열 안에 특정 문자 확인

### 📝 슈도코드
```
def solution(문자열 str1과 str2를 매개변수로 받는다):
    return 만약 str2안에 str1이 있으면 1을 반환, 없으면 0을 반환
```
```python
# 풀이 코드
